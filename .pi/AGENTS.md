...
| `shlepa run [preset] [--dry-run] [--arm <arm>] [--loop <cycles\|v3>]` | dev experiment loop: per task build env+dev images, run the agent **inside** the container (`/app`, `--network host`), score via `tests/test.sh` → `reward.txt`, log to MLflow (runs tagged `batch_id`/`mlflow_trace_id` when telemetry is on). `SLEPA_NO_DOCKER=1` switches to the legacy host-agent mode. `--arm <arm>` selects a named toolset arm (`baseline`; `+smart-grep` (code_search over ripgrep); `+sifs` (code_search over SIFS); `+forensics` (log_triage); `+mitre-kb` (mitre_kb tool + KB index prefix); `+recon` (recon tool; the prompt's recon block switches to the tool variant); `read-only` (read/write/edit + recon, no bash) — the registry in `agent/shlepa_agent/toolsets.py` is the single source); flag > `AGENT_TOOLSET` env > `baseline`, passed into the container as `AGENT_TOOLSET`, and every MLflow run is tagged `toolset=<arm>`. `--loop <cycles\|v3>` selects the pipeline regime: `cycles` (the default v5 plan→work→review pipeline) or `v3` (the budgeted single main phase + terminal commit on budget breach, `[v3loop]` budget in `agent/shlepa_agent/config.toml`); flag > `SHLEPA_LOOP` env > `cycles`, passed into the container as `SHLEPA_LOOP` (v3 only), and every v3 MLflow run is tagged `loop=v3`. Orthogonal to `--arm`. |
