...
| `shlepa run [preset] [--dry-run] [--arm <arm>]` | dev experiment loop: per task build env+dev images, run the agent **inside** the container (`/app`, `--network host`), score via `tests/test.sh` → `reward.txt`, log to MLflow (runs tagged `batch_id`/`mlflow_trace_id` when telemetry is on). `SLEPA_NO_DOCKER=1` switches to the legacy host-agent mode. `--arm <arm>` selects a named toolset arm on top of the baseline (the packaged config: the 2026-09-07 slim baseline — `recon` is the only custom tool, `read` + `recon` in plan, standard set + `recon` in work, toolless review; search/code_search/file_outline/log_triage are off): `baseline` (no-op); `+smart-grep` (re-add code_search/file_outline pinned to ripgrep); `+sifs` (re-add the pair over SIFS); `+forensics` (re-add log_triage); `+mitre-kb` (mitre_kb tool + KB index prefix); `+recon` (no-op — recon is baseline; the recon prompt block is the tool variant in the tooled phases); `read-only` (read/write/edit + recon, no bash) — the registry in `agent/shlepa_agent/toolsets.py` is the single source); flag > `AGENT_TOOLSET` env > `baseline`, passed into the container as `AGENT_TOOLSET`, and every MLflow run is tagged `toolset=<arm>`. |
