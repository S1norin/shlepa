...
5. **Recommendation (rev 3, 2026-09-03).** Keep the contest core
   (exact-ID-first + BM25 + agent re-query loop). **C′ is measured
   (`analysis/vtx-results.md`): mini-256 meets the success bar** — RRF
   hybrid paraphrase P@5 27.5% (dense 37.5%) vs BM25 20.0%, literal
   held; nano-64 marginal. Shipping mini is gated on a scope decision
   (drop the 1.19 MB verbatim-rows fallback → ≈ 9.1 MB zip). In
   parallel: semantic keyword expansion (issue #94) and the MiniLM-int8
