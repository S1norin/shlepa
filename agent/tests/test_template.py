...
def test_blocks_follow_config_order():
    ...
    out = render_user(
        cfg,
        "explore",
        {"task": "DO THE TASK", "phase_prompt": "PHASE STUFF", "extra": "EXTRA STUFF"},
    )
    assert out.index("DO THE TASK") < out.index("EXTRA STUFF") < out.index("PHASE STUFF")


def test_empty_block_dropped_with_wrappers():
    cfg = load_config()
    out = render_user(cfg, "explore", {"task": "DO THE TASK", "note": ""})
...
def test_wrapper_precedence_phase_config_over_global():
    ...
    out = render_user(cfg, "explore", {"task": "T"})
...
def test_wrapper_precedence_phase_code_over_phase_config():
    ...
    out = render_user(cfg, "explore", {"task": "T"}, phase_wrappers=code_wrapper)
...
def test_phase_template_only_applies_to_that_phase():
    ...
    out_explore = render_user(cfg, "explore", {"task": "T"})
    out_commit = render_user(cfg, "commit", {"task": "T"})
    assert "COMMIT-TASK" not in out_explore
    assert "COMMIT-TASK" in out_commit


def test_system_and_tools_go_to_system_message():
    cfg = load_config()
    system = render_system(cfg, "explore", {"system": "BASE", "tools": "bash note"})
    user = render_user(cfg, "explore", {"system": "BASE", "tools": "bash note", "task": "T"})
    assert "BASE" in system and "bash note" in system
    assert "BASE" not in user and "bash note" not in user
    assert "T" in user
