...
- ``+smart-grep``: baseline with the code_search engine pinned to
  ...
  plan — a switch away from the sifs baseline default).
- ``+sifs``: baseline with code_search over the bundled SIFS binary
  (BM25 offline — the baseline default, so the arm dedupes to the
  engine pin).
- ``+forensics``: baseline + the log_triage tool (deterministic read-only
  ...
  The baseline now ships log_triage in plan/work, so the arm mutation
  dedupes to a no-op on the packaged config.
