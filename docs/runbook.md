...
**Toolset arms.** `shlepa run --arm <arm>` selects a named toolset for the
...
evidence-triage tool; design: `research/notes/readonly-tools.md`), `+mitre-kb`
(baseline + the read-only `mitre_kb` tool over the pinned MITRE ATT&CK v19.2
knowledge base; the full technique index + alias map render into the system
prompt; design: `research/notes/mitre-rag-decision.md`). The arm is resolved as flag > `AGENT_TOOLSET` env
...
registry; it grows as new tool families land).
...
| `run --arm <arm> [preset]` | Run a batch with a named toolset arm: `baseline`, `+smart-grep`, `+sifs`, `+forensics`, `+mitre-kb` (flag > `AGENT_TOOLSET` env > baseline; tagged `toolset` in MLflow) | `shlepa run --arm +sifs all` |
