#!/usr/bin/env bash
# Verifier for bench-seccodebench-cwe94 (two-sided, SecCodeBench-V2
# protocol).
#
...
# The vendored tests are byte-identical to the upstream scenario (the
# upstream PoC suite does fail on the vulnerable baseline, so no additions
# were needed).
set -uo pipefail
