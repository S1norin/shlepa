#!/usr/bin/env python3
"""Grader for SOCBench OAuth BEC task.

Reads /app/report.json and exits 0 iff:

  verdict                    == TRUE_POSITIVE_INCIDENT
  primary_mitre_technique    contains T1114.002
  compromised_hosts          is empty (cloud-only activity)
  compromised_accounts       normalizes to exactly {'cfo'}
  key_indicators             >= 2 strings, each appearing verbatim in the
                             evidence, with >= 2 distinct hits in
                             DocuSign Pro Helper, exfil@protonmail.com, 3847
...
REQUIRED_MITRE = ['T1114.002']
REQUIRED_HOSTS = set()
REQUIRED_ACCOUNTS = {'cfo'}
IOC_KEYWORDS = {'docusign pro helper', 'exfil@protonmail.com', '3847'}
