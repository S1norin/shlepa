#!/usr/bin/env python3
"""Grader for bench-soc-ntds-vss (SOCBench SCN-012, adapted).

Reads /app/report.json and exits 0 iff:

  verdict                    == TRUE_POSITIVE_INCIDENT
  primary_mitre_technique    contains T1003.003
  compromised_hosts          normalizes to exactly {"dc01"}
  compromised_accounts       normalizes to exactly {"j.doe"}
...
REQUIRED_MITRE = "T1003.003"
REQUIRED_HOSTS = {"dc01"}
REQUIRED_ACCOUNTS = {"j.doe"}
IOC_KEYWORDS = {"vssadmin", "ntds.dit", "harddiskvolumeshadowcopy"}
...
def main() -> None:
    ...
    if not isinstance(mitre, str) or REQUIRED_MITRE not in mitre.upper():
        fail(f"primary_mitre_technique = {mitre!r}, expected to contain {REQUIRED_MITRE!r}")
