#!/usr/bin/env python3
"""Grader for bench-soc-dll-hijack-b (SOCBench SCN-039, adapted).

Reads /app/report.json and exits 0 iff:

  verdict                    == TRUE_POSITIVE_INCIDENT
  primary_mitre_technique    contains one of T1574.001
  compromised_hosts          normalizes to exactly {'ws-mkt-11'}
  compromised_accounts       normalizes to exactly {'l.martin'}
  key_indicators             >= 2 strings, each appearing verbatim in the
                             evidence, with >= 2 distinct hits in
                             dwmapi.dll, someapp, sha256
...
REQUIRED_MITRE = ['T1574.001']
REQUIRED_HOSTS = {'ws-mkt-11'}
REQUIRED_ACCOUNTS = {'l.martin'}
IOC_KEYWORDS = {'dwmapi.dll', 'sha256', 'someapp'}
