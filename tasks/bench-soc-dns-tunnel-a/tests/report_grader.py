#!/usr/bin/env python3
"""Grader for bench-soc-dns-tunnel-a (SOCBench SCN-026, adapted).

Reads /app/report.json and exits 0 iff:

  verdict                    == TRUE_POSITIVE_INCIDENT
  primary_mitre_technique    contains one of T1071.004
  compromised_hosts          normalizes to exactly {'ws-mkt-17'}
  compromised_accounts       normalizes to exactly {'e.taylor'}
  key_indicators             >= 2 strings, each appearing verbatim in the
                             evidence, with >= 2 distinct hits in
                             cloudsync-net.com, txt, cdn-284294b3
...
REQUIRED_MITRE = ['T1071.004']
REQUIRED_HOSTS = {'ws-mkt-17'}
REQUIRED_ACCOUNTS = {'e.taylor'}
IOC_KEYWORDS = {'cdn-284294b3', 'cloudsync-net.com', 'txt'}
