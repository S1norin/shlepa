# SOC ticket AL-3036 — interactive RDP logon (2026-04-15)

The `rdp-lateral-anomaly` rule fired around 09:00-09:10 UTC on 2026-04-15. The exported telemetry for the surrounding window (07:00-13:00 UTC) is attached. You are the analyst on duty.
...
- `windows_event_logs.jsonl` — Windows Event Log export for the window (one JSON object per event; process, logon, object-access, and script-block events from multiple hosts).
- `zeek_conn.jsonl` — Zeek connection log for the window (one JSON object per line).
