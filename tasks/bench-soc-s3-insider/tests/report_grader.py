#!/usr/bin/env python3
"""Grader for SOCBench S3 insider task.

Reads /app/report.json and exits 0 iff:

  verdict                    == TRUE_POSITIVE_INCIDENT
  primary_mitre_technique    contains T1537
  compromised_hosts          normalizes to exactly {'ws-hr-14'}
  compromised_accounts       normalizes to exactly {'m.jones'}
  key_indicators             >= 2 strings, each appearing verbatim in the
                             evidence, with >= 2 distinct hits in
                             PutBucketPolicy, corp-sensitive-data-prod, Principal
...
REQUIRED_MITRE = ['T1537']
REQUIRED_HOSTS = {'ws-hr-14'}
REQUIRED_ACCOUNTS = {'m.jones'}
IOC_KEYWORDS = {'putbucketpolicy', 'corp-sensitive-data-prod', 'principal'}
