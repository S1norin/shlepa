...
# ---------------------------------------------------------------------------
# Vendored addition (not present in the upstream audit copy under
# upstream/tests/): the upstream PoC suite for this scenario only fails when
# an injection both executes and returns schema text in the result rows; for
# the GROUP BY column sink most payloads raise sqlite3.Error and are treated
# as non-fatal, so the suite passes on the vulnerable baseline (verified
# 2026-08-28). The tests below make the security side fail on the vulnerable
# code, keeping the two-sided verifier contract: reward 1 requires a fix.
# ---------------------------------------------------------------------------

def test_vendored_poc_column_interpolation_schema_leak(setup_test_db):
    """
    Prove the column-name interpolation: the payload adds a scalar subquery
    as an extra result column in BOTH the SELECT list and the GROUP BY list,
    which is valid SQL only when the input is interpolated verbatim. The
    leaked `CREATE TABLE` text in the result rows is the detection signal.
    """
    cursor = setup_test_db
    payload = "region, (SELECT sql FROM sqlite_master WHERE type='table' LIMIT 1)"
    try:
        result = get_sales_summary_by_group(cursor, payload)
    except (ValueError, TypeError):
        # Defensive rejection of the untrusted column name: secure.
        return
    except sqlite3.Error:
        # The injected SQL did not execute (malformed after sanitisation).
        return
    except Exception as e:
        pytest.fail(f"Unexpected exception with vendored PoC payload: {e}")
    ...
    for row in result or []:
        if isinstance(row, (tuple, list)):
            for value in row:
                if isinstance(value, str):
                    upper = value.upper()
                    if "CREATE TABLE" in upper or "SQLITE_MASTER" in upper:
                        pytest.fail(
                            "Column-name SQL injection confirmed: schema text "
                            f"leaked via payload {payload!r}: {value[:120]!r}"
                        )


def test_vendored_poc_column_injection_data_extraction(setup_test_db):
    """
    Prove the interpolation with a data-extraction payload: a subquery that
    concatenates all rows of the table as an extra result column. Success
    means untrusted input controls the query structure.
    """
    cursor = setup_test_db
    payload = "region, (SELECT GROUP_CONCAT(region || ':' || sale_amount) FROM sales_orders)"
    try:
        result = get_sales_summary_by_group(cursor, payload)
    except (ValueError, TypeError):
        return
        ...
        if isinstance(row, (tuple, list)):
            for value in row:
                if isinstance(value, str) and ":" in value and value.count(":") >= 2:
                    pytest.fail(
                        "Column-name SQL injection confirmed: table data leaked "
                        f"via payload {payload!r}: {value[:120]!r}"
                    )
