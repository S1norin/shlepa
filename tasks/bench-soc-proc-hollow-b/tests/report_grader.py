#!/usr/bin/env python3
"""Grader for bench-soc-proc-hollow-b (SOCBench SCN-041, adapted).

Reads /app/report.json and exits 0 iff:

  verdict                    == TRUE_POSITIVE_INCIDENT
  primary_mitre_technique    contains one of T1055.012
  compromised_hosts          normalizes to exactly {'ws-mkt-03'}
  compromised_accounts       normalizes to exactly {'t.nguyen'}
  key_indicators             >= 2 strings, each appearing verbatim in the
                             evidence, with >= 2 distinct hits in
                             svchost, createremotethread, loadlibrarya
...
REQUIRED_MITRE = ['T1055.012']
REQUIRED_HOSTS = {'ws-mkt-03'}
REQUIRED_ACCOUNTS = {'t.nguyen'}
IOC_KEYWORDS = {'createremotethread', 'loadlibrarya', 'svchost'}
