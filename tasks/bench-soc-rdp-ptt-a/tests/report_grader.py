#!/usr/bin/env python3
"""Grader for bench-soc-rdp-ptt-a (SOCBench SCN-016, adapted).

Reads /app/report.json and exits 0 iff:

  verdict                    == TRUE_POSITIVE_INCIDENT
  primary_mitre_technique    contains one of T1021.001, T1550.003
  compromised_hosts          normalizes to exactly {'fs-finance-02'}
  compromised_accounts       normalizes to exactly {'k.brown'}
  key_indicators             >= 2 strings, each appearing verbatim in the
                             evidence, with >= 2 distinct hits in
                             3389, 10.10.7.100, 10.10.4.124
...
REQUIRED_MITRE = ['T1021.001', 'T1550.003']
REQUIRED_HOSTS = {'fs-finance-02'}
REQUIRED_ACCOUNTS = {'k.brown'}
IOC_KEYWORDS = {'10.10.4.124', '10.10.7.100', '3389'}
